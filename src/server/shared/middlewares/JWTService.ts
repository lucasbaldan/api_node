...
const gerarJWT = async (dados: JWTData): Promise<string | 'SECRET_NOT_FOUND' | 'SERVER_ERROR'> => {
    ...
    if (jwtsExpirados.items.length > 0) {
        jwtsExpirados.items.forEach(jwtExpirado => {
            ...
            jwtExpirado.dthr_revogacao = new Date().toString();
    ...
    const novoAcesso = await HistoricoAcessosModels.Insert({id_usuario: String(dados.userID), token: newJWT, dthr_autenticao: new Date().toString(), revogacao: false, quant_requisicoes: 1});
